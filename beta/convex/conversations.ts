...
export const ensureUserInGeneralChat = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (!generalChat) {
      // Get all existing users to add them to general chat
      const allProfiles = await ctx.db.query("userProfiles").collect();
      const allUserIds = allProfiles.map(profile => profile.userId);
      
      // Create general chat with all users
...
export const createGroupConversation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const allParticipants = [userId, ...args.participants];
...
export const addAllUsersToGeneralChat = mutation({
  args: {},
  handler: async (ctx) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Not authenticated");
    ...
    // Update general chat with all users
    await ctx.db.patch(generalChat._id, {
      participants: allUserIds,
    });
...
export const getGeneralChatMembers = query({
  ...
  handler: async (ctx, args) => {
    ...
    return members.filter(Boolean).sort((a, b) => {
      // Sort by status: online first, then away/dnd, then offline
      const statusOrder = { online: 0, away: 1, dnd: 1, offline: 2 };
